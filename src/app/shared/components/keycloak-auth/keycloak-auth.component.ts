...
export class KeycloakAuthComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.keycloakService.isTokenValid) {
      const userProfile = this.keycloakService.keycloak
      ...
      this.userStore.login({
        ...
        photoUrl:
          'https://www.google.com/url?sa=i&url=https%3A%2F%2Fwww.freepik.com%2Fvectors%2Ffemale-avatar&psig=AOvVaw3oK75XzUS5aqvyrtXuzDvY&ust=1737743315407000&source=images&cd=vfe&opi=89978449&ved=0CBQQjRxqFwoTCKCqgru8jIsDFQAAAAAdAAAAABAE',
        firstName: userProfile.firstName || 'given_name',
        lastName: userProfile.lastName || 'family_name',
        authToken:
          this.keycloakService.keycloak.authServerUrl || 'No token found',
        idToken: this.keycloakService.keycloak.idToken || 'No token found',
        authorizationCode:
          this.keycloakService.keycloak.token || 'No token found',
      });
    }
  }
  async login(): Promise<void> {
    ...
    try {
      ...
      console.error(
        'Something went wrong with ngOnInit for KeycloakAuthComponent',
        error
      );
    }
